 ...
 */
class ConfiguracionPensionController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionIndex()
    {
        if (Yii::$app->user->identity){
            if (UsuarioDetalle::find()->where(['=','codusuario', Yii::$app->user->identity->codusuario])->andWhere(['=','id_permiso',72])->all()){
                $searchModel = new ConfiguracionPensionSearch();
   ...
   public function actionView($id)
    ...
    {
       $model = new ConfiguracionPension();
    ...
    public function actionDelete($id)
    {
        try {
            ...
            $this->redirect(["configuracion-pension/index"]);
    ...
    {
        if (($model = ConfiguracionPension::findOne($id)) !== null) {
            return $model;
        }
